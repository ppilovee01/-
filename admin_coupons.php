...
// --- Logic 1: เเธิเนมเธูเธอเธ (Add) ---
if (isset($_POST['add'])) {
    ...
    if(mysqli_num_rows($check) > 0) {
        echo "<script>alert('เนเธเนดนี้มีอยูเนแล้ว!');</script>";
    } else {
...
// --- Logic 3: เตรียมเธเนอมูลเนเธเนเนเธ (Edit Fetch) ---
$edit_data = null;
if (isset($_GET['edit'])) {
    $id = $_GET['edit'];
...
<head>
    ...
    <title>จัดการเธูเธอเธ | Por Mae Bet Taled Admin</title>
...
<div class="container-fluid">
    <div class="row">
        ...
        <div class="col-md-10 p-4 p-md-5">
            <h2 class="fw-bold mb-4">จัดการเธูเธอเธสเนวเธลด</h2>
                ...
                <div class="col-md-4 mb-4">
                    <div class="card border-0 shadow-sm rounded-4 p-4 sticky-top" style="top: 20px;">
                        <h5 class="fw-bold mb-3">
                            <?php if($edit_data): ?>
                                <i class="bi bi-pencil-square text-warning"></i> เนเธเนเนเธเธูเธอเธ
                            <?php else: ?>
                                <i class="bi bi-ticket-perforated text-blue" style="color:#AEE2FF"></i> สรเนาเธเธูเธอเธเนหมเน
                            <?php endif; ?>
                        </h5>
                        ...
                        <form method="POST">
                            ...
                            <div class="mb-3">
                                <label class="small text-muted">รหัสเธูเธอเธ (Code)</label>
                                <input type="text" name="code" class="form-control text-uppercase fw-bold" placeholder="เเธเนเธ SALE2024" value="<?= $edit_data['code'] ?? '' ?>" required>
                            </div>
                            ...
                            <div class="row g-2 mb-3">
                                <div class="col-6">
                                    ...
                                    <select name="discount_type" class="form-select">
                                        <option value="fixed" <?= ($edit_data['discount_type'] ?? '') == 'fixed' ? 'selected' : '' ?>>ลดเเธเนเธเธาท (฿)</option>
                                        <option value="percent" <?= ($edit_data['discount_type'] ?? '') == 'percent' ? 'selected' : '' ?>>ลดเเธเนเธ %</option>
                                    </select>
                                </div>
                                <div class="col-6">
                                    <label class="small text-muted">มูลเธเนาสเนวเธลด</label>
                                    <input type="number" name="discount_value" class="form-control" placeholder="0" value="<?= $edit_data['discount_value'] ?? '' ?>" required>
                                </div>
                            </div>

                            <div class="mb-3">
                                <label class="small text-muted">ยอดซื้อเธัเนเธตเนำ (เธาท)</label>
                                <input type="number" name="min_spend" class="form-control" placeholder="0 = เนมเนมีเธัเนเธตเนำ" value="<?= $edit_data['min_spend'] ?? '0' ?>">
                            </div>
                            ...
                            <?php else: ?>
                                <button type="submit" name="add" class="btn btn-dark w-100 rounded-3">สรเนาเธเธูเธอเธ</button>
                            <?php endif; ?>
                        </form>
                    </div>
                </div>
                                ...
                                <tbody>
                                    ...
                                    <tr class="<?= $is_editing ?>">
                                        ...
                                        <td class="small text-muted">
                                            <?= $row['min_spend'] > 0 ? 'เธัเนเธตเนำ ฿'.number_format($row['min_spend']) : 'เนมเนมีเธัเนเธตเนำ' ?>
                                        </td>
                                        ...
                                        <td>
                                            ...
                                            <?php else: ?>
                                                <span class="badge bg-success">เนเธเนเธาเธได้</span>
                                            <?php endif; ?>
                                        </td>
                                        <td class="text-end">
                                            <a href="?edit=<?= $row['id'] ?>" class="btn btn-light btn-sm text-primary rounded-circle shadow-sm me-1"><i class="bi bi-pencil-fill"></i></a>
                                            <a href="?del=<?= $row['id'] ?>" class="btn btn-light btn-sm text-danger rounded-circle shadow-sm" onclick="return confirm('ลเธเธูเธอเธนี้?');"><i class="bi bi-trash-fill"></i></a>
                                        </td>
                                    </tr>
                                    <?php endwhile; ?>
                                </tbody>
